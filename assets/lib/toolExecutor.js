...
export async function executeTool(name, input, cfg) {
    ...
    try {
        if (name === 'search_content_advanced') {
            ...
            if (data.items) {
                data.items = data.items.map((item) => { const emb = item.embedded || {}; const briefs = emb.taxonomyCategoryBriefs || []; const taxonomyCategories = briefs.length > 0 ? briefs.map((b) => ({ id: b.taxonomyCategoryId, name: b.taxonomyCategoryName })) : (emb.taxonomyCategories || []); let contentId = emb.id || item.id || item.entryClassPK || null; if (!contentId && item.itemURL) { const m = item.itemURL.match(/\/structured-contents\/(\d+)/); if (m) contentId = m[1]; } return { id: contentId, title: item.title || emb.title || emb.headline || null, friendlyUrlPath: emb.friendlyUrlPath || null, datePublished: emb.datePublished || item.dateModified || null, description: emb.description || item.description || null, contentStructureName: emb.contentStructureName || null, taxonomyCategories, keywords: emb.keywords || [], score: item.score || null, itemURL: item.itemURL || null, ...(doEmbed ? { _embedded: emb } : {}) }; });
